...
export default async function handler(req, res) {
    ...
    async function finalizeWithdrawal({ w, approve, txHash, chatId, msgId }) {
        ...
        const notif = approve
            ...
            : `❌ <b>Withdrawal Rejected.</b>\n${(w.wtcAmount || 0).toLocaleString()} CN has been refunded to your balance.`;
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('addtask_cat_')) {
            ...
            await tgSend(chatId, '📋 <b>Add Task — Step 5/6</b>\n\nReward currency?', {
                reply_markup: { inline_keyboard: [[{ text: 'CN', callback_data: 'addtask_cur_wtc' }, { text: 'USDT', callback_data: 'addtask_cur_usdt' }]] },
            });
            return res.status(200).json({ ok: true });
        }
        ...
        // ── Add Promo wizard ── code is auto-generated (random 6 digits), no
        // more typing one in — see generatePromoCode() above.
        if (data === 'a_addpromo') {
            let code;
            try {
                code = await generatePromoCode(promos);
            } catch (e) {
                await tgSend(chatId, `❌ ${e.message}`, { reply_markup: backKb });
                return res.status(200).json({ ok: true });
            }
            await setAdminState(db, fromId, { step: 'addpromo_amount', promo: { code } });
            await tgSend(chatId, `🎟 <b>Add Promo — Step 1/2</b>\n\nGenerated code: <code>${code}</code>\n\nSend the reward amount (CN).`, { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
    ...
    if (update.message) {
        ...
        if (fromId === ADMIN_ID) {
            ...
            switch (st.step) {
                ...
                // ── Add Promo wizard — code was generated already in the
                // a_addpromo callback above, so this now starts at the reward. ──
