...
// ⚠️ WIDENED (ads update) — the reward is no longer POSTed the instant the
// round ends: the game-over screen now shows a "Watch Ad & Claim" button
// (see index.html claimGameReward()) and the 'finish' call only fires once
// the player has watched a Monetag/GigaPub rewarded ad and tapped Claim.
// That adds real, variable human + ad-watch time on top of the round
// itself, so the old 20s buffer (sized only for network latency) is far
// too tight and would reject legitimate claims as 'session_expired'. This
// does NOT weaken the anti-speedhack floor below (CLOVER_GAME_MIN_MS_PER_CLOVER
// still rejects implausibly fast clover counts) — it only widens the upper
// bound on how long a player may take to actually claim.
export const CLOVER_GAME_SESSION_GRACE_SECONDS = 900;
