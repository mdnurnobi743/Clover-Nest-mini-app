...
//   • adWatchLogs.watchedAt      → 7-day ad-watch log auto-delete
//     (see api/earn.js handleAdWatchClaim)
//   • cloverSessions.createdAt   → 3-day game-session auto-delete
//     (see api/clover.js — this collection previously had NO cleanup at all)

import { MongoClient } from 'mongodb';
import { AD_WATCH_LOG_TTL_SECONDS, CLOVER_SESSION_TTL_SECONDS } from './constants.js';
...
async function ensureIndexes(db) {
    await Promise.all([
        ...
        // ⚠️ NEW — TTL cleanup system (see lib/constants.js "Storage TTL / cap
        // system" note for why these two are real TTL indexes while spin/game
        // history is handled a different way).
        db.collection('adWatchLogs').createIndex(
            { watchedAt: 1 },
            { expireAfterSeconds: AD_WATCH_LOG_TTL_SECONDS } // 7 days
        ),
        db.collection('cloverSessions').createIndex(
            { createdAt: 1 },
            { expireAfterSeconds: CLOVER_SESSION_TTL_SECONDS } // 3 days — was never cleaned up before this
        ),
    ]);
}
