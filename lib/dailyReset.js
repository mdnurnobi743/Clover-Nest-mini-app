...
import { todayBD, dailyResetFields, DAILY_FREE_SPINS, DAILY_CLOVER_GAMES } from './constants.js';

export async function ensureDailyReset(users, userId) {
    const today = todayBD();

    const user = await users.findOne({ _id: userId }, { projection: { lastResetDate: 1, spinsRemaining: 1, cloverGamesRemaining: 1 } });
