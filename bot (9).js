...
function renderUserLookup(u, withdrawalsList = []) {
    ...
    const text =
        ...
        `✅ Tasks Completed: <b>${(u.completedTasks || []).length}</b>${u.tasksRequirementMet ? ' (requirement waived ✅)' : ''}\n` +
    ...
    const options = { reply_markup: { inline_keyboard: [
        ...
        // Manual override for the "8 tasks" withdraw gate — see lib/taskRequirement.js.
        // Normally adaptive (never asks for more than what's actually available), but
        // this lets the admin unblock one specific stuck user immediately if needed.
        [u.tasksRequirementMet ? { text: '↩️ Re-enable Task Requirement', callback_data: `taskreq_off_${u._id}` } : { text: '✅ Waive Task Requirement', callback_data: `taskreq_on_${u._id}` }],
...
async function handleUpdate(req, res) {
    ...
    async function finalizeWithdrawal({ w, approve, txHash, chatId, msgId }) {
        ...
        const u = await users.findOne({ _id: w.userId }, { projection: { referralCount: 1, withdrawalCount: 1, telegramUsername: 1, firstName: 1 } });
        const wUsername = cleanUsername(w.username) || cleanUsername(u?.telegramUsername); // old requests may have 'N/A' saved — fall back to the profile
        const processedText =
            `💸 <b>Withdrawal Request</b>\n\n` +
            `👤 <b>Telegram:</b> ${adminUserTag(w.userId, wUsername, w.firstName || u?.firstName)}\n` +
            `🆔 ID: <code>${w.userId}</code>\n` +
            (w.wtcAmount ? `🪙 CN: <b>${w.wtcAmount.toLocaleString()}</b>\n` : '') +
            `💰 Amount: <b>${w.cashAmount.toFixed(4)} ${w.currency}</b>\n` +
            `📤 Method: <b>${w.method}</b>\n` +
            `📍 Address: <code>${escHtml(w.details)}</code>\n` +
    ...
    if (update.callback_query) {
        ...
        if (data === 'a_pending') {
            ...
            // Look up the CURRENT username/name of everyone in the list so requests
            // saved with 'N/A' still show the user's real Telegram @username.
            const pendingUsers = await users.find({ _id: { $in: pending.map((x) => x.userId) } }, { projection: { telegramUsername: 1, firstName: 1 } }).toArray();
            const pendingUserMap = new Map(pendingUsers.map((x) => [x._id, x]));
            for (const w of pending) {
                const pu = pendingUserMap.get(w.userId);
                const text =
                    `💸 <b>Withdrawal Request</b>\n\n` +
                    `👤 <b>Telegram:</b> ${adminUserTag(w.userId, cleanUsername(w.username) || cleanUsername(pu?.telegramUsername), w.firstName || pu?.firstName)}\n` +
                    `🆔 ID: <code>${w.userId}</code>\n` +
        ...
        if (data.startsWith('taskreq_on_') || data.startsWith('taskreq_off_')) {
            const waive = data.startsWith('taskreq_on_');
            const uid = data.replace(waive ? 'taskreq_on_' : 'taskreq_off_', '');
            await users.updateOne({ _id: uid }, { $set: { tasksRequirementMet: waive } });
            await tgAnswerCallback(cb.id, waive ? '✅ Task requirement waived — they can withdraw now' : '↩️ Task requirement re-enabled', true);
            const u = await users.findOne({ _id: uid });
            if (u) { const wList = await withdrawals.find({ userId: uid }).toArray(); await tgSend(chatId, ...renderUserLookup(u, wList)); }
            return res.status(200).json({ ok: true });
        }
